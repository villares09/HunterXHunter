...
import { useForest, plant, erase } from "@/data/forestStore";
...
function TopCamera({ cx, cz }: { cx: number; cz: number }) {
  ...
  useEffect(() => {
    camera.position.set(cx, 220, cz + 200);
...
export function ForestScene() {
  ...
  return (
    <>
      ...
      {/* terreno clickeable para pintar */}
      ...
      {cursor && (
        <mesh position={cursor} rotation={[-Math.PI / 2, 0, 0]}>
          ...
          <meshBasicMaterial color={mode === "plant" ? "#4ade80" : "#ff5252"} transparent opacity={0.8} side={THREE.DoubleSide} depthTest={false} />
        </mesh>
      )}
    </>
  );
